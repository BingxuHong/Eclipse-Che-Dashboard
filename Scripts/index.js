    ...
    Workspace.GetAll(function (data) {
        ...
        if (data && data.length > 0) {
            ...
            $("input[id^='btnStart']").on("click", function () {
                ...
                if (status === "STOPPED") {
                    Workspace.Start(id);
                    $($("#trWorkspace_"+ id).children()[4]).html("");

                    var postData = {
                                        "uuid":"82beef55-84e7-422a-cf64-17c49227520b",
                                        "method":"POST","path":null,
                                        "headers":[{"name":"x-everrest-websocket-message-type","value":"subscribe-channel"}],
                                        "body":"{\"channel\":\"workspace:"+id+":ext-server:output\"}"
                                    };
                    wsList[id].send(JSON.stringify(postData));
                }
            });
...
function fn_WorkspaceStatusSetting(workspaceId) {
    if ("WebSocket" in window) {
        ...
        wsList[workspaceId] = new WebSocket(url);

        wsList[workspaceId].onopen = function () {
            ...
            wsList[workspaceId].send(JSON.stringify(postData));
        };

        wsList[workspaceId].onmessage = function (evt) {
            var received_msg = JSON.parse(evt.data);
            if (received_msg && received_msg.responseCode === 0 && received_msg.headers[0].value === ("workspace:" + workspaceId)) {
            ...
            } else if (received_msg && received_msg.responseCode === 0 && received_msg.headers[0].value === ("workspace:" + workspaceId + ":ext-server:output")) {
                var consoleMsg = received_msg.body;
                $($("#trWorkspace_" + workspaceId).children()[4]).append("<span>" + consoleMsg + "</span><br/>");
            }

        };

        wsList[workspaceId].onclose = function () {
            // websocket is closed.
